...
func calculate(_ args: [String]) -> Int {
    if args.count == 3 {
        guard let num1 = Int(args[0]),
              let num2 = Int(args[2]) else {return 0}
    ...
    // Handling count, avg, and fact operations
    switch args.last {
    case "count":
        return args.count - 1
    case "avg":
    ...
    case "fact":
        guard let num = Int(args[0]), args.count == 2 else {return 0}
        if num == 0 {
            return 1
        }
        var factorial = 1
...
func calculate(_ args: [String]) -> Double {
    if args.count == 3 {
        guard let num1 = Double(args[0]),
              let num2 = Double(args[2]) else {return 0}
...
func calculate(_ arg: String) -> Double {
    let components = arg.split(separator: " ").map(String.init)
